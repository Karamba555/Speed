...
(function(window, document, undefined) {
	...
	var LuCI = Class.extend(/** @lends LuCI.prototype */ {
		...
		require: function(name, from) {
			...
			var shortName = name.replace(/-[0-9]+$/, '');

			/* Class already loaded */
			if (classes[shortName] != null) {
				/* Circular dependency */
				...
				return Promise.resolve(classes[shortName]);
			}

			if (name == shortName) {
               			url = '%s/%s-DATESWAP.js%s'.format(env.base_url, name.replace(/\./g, '/'), (env.resource_version ? '?v=' + env.resource_version : ''));  
            		} else {
  			  	url = '%s/%s.js%s'.format(env.base_url, name.replace(/\./g, '/'), (env.resource_version ? '?v=' + env.resource_version : ''));
            		}
			from = [ shortName ].concat(from);
			...
			var compileClass = function(res) {
				...
				for (var i = 0, off = -1, prev = -1, quote = -1, comment = -1, esc = false; i < source.length; i++) {
					...
					if (esc) {
						...
						if (m) {
							var dep = m[1];
                            				dep = dep.replace(/-[0-9]+$/, '');
                            				var as = m[2] || dep.replace(/[^a-zA-Z0-9_]/g, '_');
				...
				return Promise.all(depends).then(function(instances) {
					...
					_factory.displayName = toCamelCase(shortName + 'ClassFactory');
					...
					if (_class.displayName == 'AnonymousClass')
						_class.displayName = toCamelCase(shortName + 'Class');
			...
			/* Request class file */
			classes[shortName] = Request.get(url, { cache: true }).then(compileClass);
