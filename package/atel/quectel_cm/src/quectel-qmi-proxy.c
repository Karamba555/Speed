                    ...
                    if (qmi_con->ClientFd == qmi_msg->ClientFd) {
                        ...
                        if (pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType == QMICTL_GET_CLIENT_ID_RESP)
                            get_client_id(qmi_con, &pQMI->CTLMsg.GetClientIdRsp);                                                        
                                ...
                                (pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType == QMICTL_REVOKE_CLIENT_ID_IND)) {
                            release_client_id(qmi_con, &pQMI->CTLMsg.ReleaseClientIdRsp);
                            if (pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType == QMICTL_REVOKE_CLIENT_ID_IND)
                                modem_reset_flag = 1;
                        }
                        else {
                        }
...
//Maximum number of datagrams in a single aggregated packet on downlink
    ...
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationMaxDatagramsTlv.Value =  cpu_to_le32(rx_urb_size/1024);  //cpu_to_le32((rx_urb_size>2048)?(rx_urb_size/1024):1);
...
static void *qmi_proxy_loop(void *param)
{
    ...
    dprintf("%s enter thread_id %ld\n", __func__, (long int)pthread_self());
...
void set_usbnet()
{
	...
	status = at_handle(dev_fd, "at+qcfg=\"usbnet\",0\r", tmpbuf, 5);
