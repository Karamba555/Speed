...
static void ql_prepare_daemon(void) {
    ...
    if (pipe(daemon_pipe_fd) < 0) {
        dbg_time("%s Failed to create daemon_pipe_fd: %d (%s)", __func__, errno, strerror(errno));
        return;
    }
...
static int usage(const char *progname) {
     ...
     dbg_time("--syslog                               Send debug messages to syslog too");
	 dbg_time("[Examples]");
...
static void reboot_lte()
{
	int dev_fd=-1, fail_time=0;
	char	tmpbuf[MAX_BUF_SIZE]	= { '\0' };
	STATUS	status		= STATUS_OK;

	if ((OpenDev(&dev_fd, LTEDEV)==-1)
		|| (SetSpeed(dev_fd, 9600)==-1)
		|| (SetParity(dev_fd, 8, 1, 'N'))==-1) {
		system("gpio c aw12-power; sleep 1; gpio w aw12-power &");
		return ;
	}
	status = at_handle(dev_fd, "AT+CFUN=0\r", tmpbuf, 5);
	while ((STATUS_OK != status) || !strstr(tmpbuf, "OK")) {
		status = at_handle(dev_fd, "AT+CFUN=0\r", tmpbuf, 5);
		fail_time++;
		if (fail_time > 3) {
			goto out;
	...
	fail_time = 0;
	status = at_handle(dev_fd, "AT+CFUN=1\r", tmpbuf, 5);
	while ((STATUS_OK != status) || !strstr(tmpbuf, "OK")) {
		status = at_handle(dev_fd, "AT+CFUN=1\r", tmpbuf, 5);
		fail_time++;
		if (fail_time > 3) {
	...
	fail_time = 0;
	status = at_handle(dev_fd, "AT+QPOWD\r", tmpbuf, 5);
	while ((STATUS_OK != status) || !strstr(tmpbuf, "OK")) {
		status = at_handle(dev_fd, "AT+QPOWD\r", tmpbuf, 5);
		fail_time++;
		if (fail_time > 3) {
...
int qmi_main(PROFILE_T *profile)
{
    ...
    UCHAR  IPv4ConnectionStatus = 0xff; //unknown state
    UCHAR  IPv6ConnectionStatus = 0xff; //unknown state
    unsigned SetupCallFail = 0;
	unsigned CheckSimFail = 0;
...
__main_loop:
    /* try to recreate FDs*/
    if (socketpair( AF_LOCAL, SOCK_STREAM, 0, signal_control_fd) < 0 ) {
        dbg_time("%s Failed to create main_control_fd: %d (%s)", __func__, errno, strerror(errno));
        return -1;
    }
    ...
    while (!profile->qmichannel) {
        ...
        if (!qmidevice_detect(qmichannel, usbnet_adapter, sizeof(qmichannel))) {
            dbg_time("Detected QMI channel: %s", qmichannel);
            dbg_time("Detected network adapter: %s", usbnet_adapter);
    ...
    while (qmierr == QMI_ERR_OP_DEVICE_UNSUPPORTED) {
        sleep(1);
        qmierr = requestGetSIMStatus(&SIMStatus);
        write_wan_status(profile, &WANSTATUS);
    }
...
#endif
    requestGetSerialNumbers();
#endif

//    requestGetSignalStrength();
//    requestGetTxRxInfo();
//    requestGetRFBandInfo();
    requestGetSystemSelectionPreference();
//    requestGetCellLocationInfo();
    ...
    while (true)
    {
        ...
        if ((now - last_status) > 15) {
            ...
            // Only report dynamic values on first connection
            requestGetSignalStrength(firstReport);
            requestGetTxRxInfo(firstReport);
            //requestGetRFBandInfo(firstReport);
            //requestGetCellLocationInfo(firstReport);
            //firstReport = false;
                    ...
                    switch (signo)
                    {
                        case SIG_EVENT_START:
                            ...
                            if ((profile->enable_ipv4 && IPv4ConnectionStatus ==  QWDS_PKT_DATA_DISCONNECTED)
                                ...
                                const unsigned long allow_time[] = {5, 10, 20, 25, 30};
                                ...
                                SetupCallFail++;
                                dbg_time("try to requestSetupDataCall %lu seconds later", SetupCallAllowTime);
                        ...
                        case SIG_EVENT_CHECK:
                            if (profile->enable_ipv4) {
                                ...
                                // local IP is different than remote IP
                                if (QWDS_PKT_DATA_CONNECTED == IPv4ConnectionStatus && check_ipv4_address(profile) == 0) {
                                    requestDeactivateDefaultPDP(profile, IpFamilyV4, 0);
                        ...
                        case SIG_EVENT_STOP:
                            ...
                            if (profile->enable_ipv6 && IPv6ConnectionStatus ==  QWDS_PKT_DATA_CONNECTED) {
                                    requestDeactivateDefaultPDP(profile, IpFamilyV6, 0);
                            }
                            usbnet_link_change(0, profile);
                    ...
                    switch (triger_event) {
                        ...
                        case RIL_UNSOL_DATA_CALL_LIST_CHANGED:
                            if (IPv4ConnectionStatus ==  QWDS_PKT_DATA_CONNECTED || IPv6ConnectionStatus ==  QWDS_PKT_DATA_CONNECTED)
                                SetupCallAllowTime = clock_msec() + 1000; //from connect -> disconnect, do not re-dial immediately, wait for network to stabilize 
                                    send_signo_to_main(SIG_EVENT_CHECK);
                        break;
                        ...
                        case MODEM_REPORT_RESET_EVENT:
                        {
                            ...
                            dbg_time("quit QMI thread and wait %us and try to restart", time_to_wait);
                            main_send_event_to_qmidevice(RIL_REQUEST_QUIT);
                            /** NOTICE
                             * DO NOT CALL usbnet_link_change(0, profile) DIRECTLLY
                             * for, the modem may go into wrong state(only ttyUSB0 left) and won't go back
                             */
                            usbnet_link_state(0);
...
/**
 * Lookup and copy individual UCI value
 */ 

static bool lookup_value(const char *name, char *value, size_t len, struct uci_context *uci, struct uci_ptr *p) {
  bool result = true;
  ...
  if (uci_lookup_ptr(uci, p, name_copy, true) != UCI_OK || !((*p).o)) {
    result = false;
    printf("Lookup for name: '%s' failed\n", name);
  } else {
    printf("Lookup for name: '%s' = '%s'\n", name, (*p).o->v.string);
      
    // Special processing for URL   
    if (strcmp(name, "awc_cloud.cloud.host") == 0 && !strstr((*p).o->v.string, "://")) {
      // Append https if needed
     
      strcpy(value, "https://");
      strncpy(value + 8, (*p).o->v.string, len - 1 - 8);
        
    } else {
      strncpy(value, (*p).o->v.string, len - 1);
    }
...
/**
 * System configuration/status values 
 */
static bool update_systemconfig(struct wan_status *status) {
     
  // Get EI serial number
  FILE *serialNumber = fopen("/tmp/nextivity/serialNumber", "r");
  if (!serialNumber) {
    fprintf(stderr, "Failed to locate system serial number\n");
  } else {
    size_t num = fread(status->serialNumber, 1, sizeof(status->serialNumber) - 1, serialNumber); 
      
    if (num > 1) {
      // Trim LF
      status->serialNumber[num - 1] = '\0';
...
#if 0  
  // Get Linux version
  const char *const args[3] = { "uname", "-r", NULL };
  int bytes = Awc_Exec("/bin/uname", args, 1000, 100, si->pinflux_info->linuxversion, INFLUXINFOSIZE - 1);
    
  if (bytes < 1) {
    fprintf(stderr, "Determining Linux version failed\n");   
  } else {
    // Remove LF
    si->pinflux_info->linuxversion[bytes - 1] = '\0';
    //printf("Linux Version: '%s'\n", influx_info.linuxversion);  
  }
#endif  
   
  // Get Firmware version  
  FILE *buildInfo = fopen("/etc/nextivity_build_info", "r");
  
  if (!buildInfo) {
    fprintf(stderr, "Failed to open EI build info file\n");
  } else {  
    char buffer[1024];
    memset(buffer, 0, sizeof(buffer));
  
    size_t len = fread(buffer, 1, sizeof(buffer) - 1, buildInfo);
    ...
    if (len > 0) {
       ...
       if (version) {
         ...
         if (lf) {
           *lf = '\0';
           char *swversion = version + 9;
           
           printf("Software Version: '%s'\n", swversion);
           strncpy(status->softwareVersion, swversion, sizeof(status->softwareVersion) - 1);
           //UpdateVariableStatus(si->pstatus_values, ES_PCSWV, swversion); 
         }
      }
    }
    fclose(buildInfo);
...
//  char result[INFLUXINFOSIZE];
  ...
  lookup_value("network.lan.ipaddr", status->lanip, sizeof(status->lanip), uci, &p);
...
int main(int argc, char *argv[])
{
    ...
    dbg_time("Quectel_QConnectManager_Linux_V1.5.1");
    ...
    WANSTATUS.c0rxpower = NAN; 
    WANSTATUS.c0ecio    = NAN;
    WANSTATUS.c0rsrp    = NAN;
    WANSTATUS.c0phase   = NAN;
    
    WANSTATUS.c1rxpower = NAN; 
    WANSTATUS.c1ecio    = NAN;
    WANSTATUS.c1rsrp    = NAN;
    WANSTATUS.c1phase   = NAN;
    
    WANSTATUS.txpower   = NAN;
    
    update_systemconfig(&WANSTATUS);
    ...
    if (do_syslog) {
      dbg_time("Syslog debug enabled");
      openlog("quectel-CM", 0, LOG_DAEMON);
    }
