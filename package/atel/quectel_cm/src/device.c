...
BOOL qmidevice_detect(char *qmichannel, char *usbnet_adapter, unsigned bufsize) {
    ...
    while ((ent = readdir(pDir)) != NULL)  {
        ...
        if (idVendor != 0x05c6 && idVendor != 0x2c7c && idVendor != 0x1bc7)
            continue;
        
        dbg_time("Find %s/%s idVendor=0x%x idProduct=0x%x", rootdir, ent->d_name, idVendor, idProduct);

        /* get network interface */
        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.2/net", rootdir, ent->d_name);
        ...
        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.2/GobiQMI", rootdir, ent->d_name);
        if (access(pl->path, R_OK)) {
            snprintf(pl->path, sizeof(pl->path), "%s/%s:1.2/usbmisc", rootdir, ent->d_name);
            if (access(pl->path, R_OK)) {
                snprintf(pl->path, sizeof(pl->path), "%s/%s:1.2/usb", rootdir, ent->d_name);
                if (access(pl->path, R_OK)) {
                    dbg_time("no GobiQMI/usbmic/usb found in %s/%s:1.2", rootdir, ent->d_name);
                    continue;
                }
            }
        }
    ...
    if (qmichannel[0] == '\0') {
        dbg_time("Network interface not found");
        goto error;
    }
    
    if (usbnet_adapter[0] == '\0') {
        dbg_time("QMI device not found");
        goto error;
    }
