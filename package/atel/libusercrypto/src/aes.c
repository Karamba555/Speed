...
int aes_encrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	...
	for (idx = 0; idx < blocks; idx++) {
		...
		aes_encrypt_libusercrypto(buf_in, buf_out, key, keysize);
...
int aes_decrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	...
	for (idx = 0; idx < blocks; idx++) {
		memcpy(buf_in, &in[idx * AES_BLOCK_SIZE], AES_BLOCK_SIZE);
		aes_decrypt_libusercrypto(buf_in, buf_out, key, keysize);
...
void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	...
	if (in_len > AES_BLOCK_SIZE) {
		for (idx = 0; idx < last_block_length; idx += AES_BLOCK_SIZE) {
			aes_encrypt_libusercrypto(iv_buf, out_buf, key, keysize);
...
void aes_encrypt_libusercrypto(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	BYTE state[4][4];
...
void aes_decrypt_libusercrypto(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	BYTE state[4][4];
...
void print_hex(BYTE str[], int len)
{
	...
	for(idx = 0; idx < 32; idx++)
		...
		LIBDBG_MSG("%02X ", str[idx]);

        }
       LIBDBG_MSG("\n");
}
int aes_cbc_test(unsigned char filename[] ,unsigned long offset,unsigned long len)
{
        unsigned char cmd[32] ={'\0'};
        WORD key_schedule[60];
        unsigned char enc_buf_again[256]={'\0'};
        //unsigned char plaintext[32]={'\0'};
	//unsigned char enc_buf[128];
        unsigned char iv[16] = {0};
        unsigned char key[40]={""};
        strcpy(key,"0f1571c947d9e8590cb7add6af7f6798");
        key[32] = '\0';
        int i;
        ...
        len = len-64;
        LIBDBG_MSG("%s(%d)filename=%s len=%lu\n",__FUNCTION__,__LINE__,filename,len);
        unsigned char *plaintext = malloc(len);
        rewind(file); 
        fseek(file, offset+64, SEEK_SET);
        fread(plaintext, 1, len, file);
        fclose(file);
	...
	aes_key_setup(key, key_schedule, 256);
        ...
        #if 0
        LIBDBG_MSG("%s(%d)modify newlen=%lu\n",__FUNCTION__,__LINE__,len);
	LIBDBG_MSG("* CBC mode:\n");
	LIBDBG_MSG(  "Key          : ");
	print_hex(key, 32);
	LIBDBG_MSG("\nIV           : ");
	...
	LIBDBG_MSG("\nend: ");
	print_hex(plaintext+len-32, 32);
        #endif

        unsigned char *enc_buf = malloc(len+1);
	aes_decrypt_cbc(plaintext, len, enc_buf, key_schedule, 256, iv);
        free(plaintext);
        #if 1
        for(i = 1; i < 16; i++)
        {
...
#if 0
       LIBDBG_MSG("%s(%d)reduce newlen=%lu\n",__FUNCTION__,__LINE__,len);
	LIBDBG_MSG("\nstart ");
	LIBDBG_MSG("\ndecrypted    : ");
	print_hex(enc_buf, 32);
	LIBDBG_MSG("\nend ");
	LIBDBG_MSG("\ndecrypted    : ");
	print_hex(enc_buf+len-32, 32);
        LIBDBG_MSG("\n");
#endif

        fileout = fopen(IMAGE_DECRYPTED_NAME,"w");
        fwrite(enc_buf, 1, len, fileout);
        fclose(fileout);
        free(enc_buf);
	return(0);
}

unsigned long calculate_file_size(FILE *fp)
{
	unsigned long cnt = 0;
...
int read_file_context_to_mem(FILE *fp, char **ptr_buf, unsigned int length)
{
	unsigned long cnt = 0;
	char buf[256] = {0};
	...
	*ptr_buf = malloc(length);
	if (*ptr_buf == NULL) return cnt;

	memset(*ptr_buf, 0, length);
	cnt = fread(*ptr_buf, 1, length, fp);

	return cnt;
}

int write_mem_to_file_context(FILE *fp, char **ptr_buf, unsigned int length)
{
	unsigned long cnt = 0;
	
	if (fp == NULL)	return cnt;

	cnt = fwrite(*ptr_buf, 1, length, fp);

	return cnt;
}

size_t encrypt_mem_with_aes_cbc(const char **ori_ctx, unsigned int length, char **new_ctx)
{
	size_t cnt = 0;
	char *plain_txt = NULL;
	char *encrypt_txt = NULL;
	unsigned char key[40] = {"0f1571c947d9e8590cb7add6af7f6798"};
	unsigned int key_schedule[60] = {0};
	unsigned char iv[AES_BLOCK_SIZE] = {0};

	*new_ctx = malloc(length);
	if (*new_ctx == NULL) return cnt;

	memset(*new_ctx, 0, length);
	aes_key_setup(key, key_schedule, 256);
	aes_encrypt_cbc(*ori_ctx, length, *new_ctx, key_schedule, 256, iv);

	return length;

}

size_t decrypt_mem_with_aes_cbc(const char **ori_ctx, unsigned int length, char **new_ctx)
{
	size_t cnt = 0;
	unsigned char i = 0;
	char *plain_txt = NULL;
	...
	*new_ctx = malloc(length);
	...
	memset(*new_ctx, 0, length);
	aes_key_setup(key, key_schedule, 256);
	aes_decrypt_cbc(*ori_ctx, length, *new_ctx, key_schedule, 256, iv);

	/* should remove the rest of '0' at end of buf
	** because it will fill at end of buf with spare '0' at encrypting */
	for (i = 1; i < AES_BLOCK_SIZE; i++) {
		if ((*new_ctx)[length-i] != 0)
			break;
	}
	cnt = length-i+1;
...
int libuser_crypt_file_type(const char *file_src, char *file_dst, enum crypt_mode crypt_type)
{
	char *encry_ctx_ori = NULL;
	char *encry_ctx_new = NULL;
	FILE *fp_r = NULL;
	FILE *fp_w = NULL;
	unsigned long file_size = 0;
	crypt_test_cb crypt_cb = NULL;

	fp_r = fopen(file_src, "r");
	fp_w = fopen(file_dst, "w");
	
	if (fp_r == NULL) goto error;
	if (fp_w == NULL) goto error;

	switch (crypt_type) {
		case AES_CBC_Encrypt:
			crypt_cb = encrypt_mem_with_aes_cbc;
			break;
		case AES_CBC_Decrypt:
			crypt_cb = decrypt_mem_with_aes_cbc;
			break;
	}

	file_size = calculate_file_size(fp_r);
	if (file_size % AES_BLOCK_SIZE != 0)
		file_size = (file_size / AES_BLOCK_SIZE + 1) * AES_BLOCK_SIZE;

	read_file_context_to_mem(fp_r, &encry_ctx_ori, file_size);
	if (crypt_cb)
		file_size = crypt_cb(&encry_ctx_ori, file_size, &encry_ctx_new);
	write_mem_to_file_context(fp_w, &encry_ctx_new, file_size);
	
error:
	if (encry_ctx_ori) free(encry_ctx_ori);
	if (encry_ctx_new) free(encry_ctx_new);
	if (fp_r) fclose(fp_r);
	if (fp_w) fclose(fp_w);
