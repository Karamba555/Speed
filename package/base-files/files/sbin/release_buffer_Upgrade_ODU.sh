...
aim stop 2>/dev/null
killall -9 aimd.s udhcpc easycwmpd dhcplisttool atelIDURM ltemanage lighttpd host_atc syslogd dnsmasq wanfailovertool 2>/dev/null
...
cd /bin;rm -rf ated ntpclient config_generate sed_atel 2>/dev/null 
cd /usr/bin; rm -rf aim* regVoip client_thread  cfg_backup cfg_upgrade diag_transfer factory* fw_upgrade init_system  serial_tool *emoteUpgrade* lte_cpe_Upgrade_local ping_watchdog    miniupnpd iperf atelIDURM  miniupnpd curl  ip dnsmasq   wps_tool wanfailovertool wanfailover atelIDURM diagtransfer dhcplisttool iperf3 htop 2>/dev/null 
...
rm -rf /etc_ro/lighttpd/www/ 2>/dev/null
