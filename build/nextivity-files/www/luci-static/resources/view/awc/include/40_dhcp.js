...
return baseclass.extend({
	...
	renderLeases: function(data) {
		var leases = Array.isArray(data[0].dhcp_leases) ? data[0].dhcp_leases : [],
		    ...
		    isReadonlyView = true; // !L.hasViewPermission();
		...
		var table = E('table', { 'class': 'table leases' }, [
		...
		return E([
			E('h6', _('Active DHCPv4 Leases')),
			table,
/*            E('hr', { class: 'status-section' }),*/
			E('h6', _('Active DHCPv6 Leases')),
			table6
		]);
	},
	...
	render: function(data) {
        ...
        if (bridge_mode == "on") {
          return  E('em', _('No local DHCP servers in Passthrough Mode<br>&nbsp;<br>')); 
