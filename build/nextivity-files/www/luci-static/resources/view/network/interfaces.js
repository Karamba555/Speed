...
function render_status(node, ifc, with_device) {
    ...
    // Hide netmask for wwan
    var ip6pd = changecount ? null : ifc.getIP6Prefix();

    if (ifc.getName() == "wwan") {
      if (ipaddrs[0] != undefined) {
        ipaddrs[0] = (ipaddrs[0].split('/'))[0];
      }

      if (ip6addrs[0] != undefined) {
        ip6addrs[0] = (ip6addrs[0].split('/'))[0];
      }
...
function render_ifacebox_status(node, ifc) {
    ...
    if (!dev && ifc.getName() == "wwan") {
        name = "wwan";
    } else {
        name = dev ? dev.getName() : E('em', _('Not present'));
    }
	...
	poll_status: function(map, networks) {
		...
		for (var i = 0; i < networks.length; i++) {
			...
			if (dsc.hasAttribute('reconnect')) {
                ...
                if (ifc.getName() == "wwan") {
                    dom.content(dsc, [
                        E('em', _('Bridge Mode - No configuration possible'))
                        ]);
