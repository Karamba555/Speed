...
var radioFrequencies = [];
var radioFRequenciesOld = [];
...
var authserverVar    = [];
var authserverVarOld = [];

var authportVar    = [];
var authportVarOld = [];

var authsecretVar    = [];
var authsecretVarOld = [];
    ...
    poll_status: function(map, data) {
        ...
        for (var i = 0; i < rows.length; i++) {
            var section_id = rows[i].getAttribute('data-sid'),
                ...
                busy = true; //btns[0].classList.contains('spinning') || btns[1].classList.contains('spinning') || btns[2].classList.contains('spinning');
...
/*              dom.content(badge, render_radio_badge(radioDev));
              ...
              console.log("call render_network_status");
            ...
            btns[0].hidden = btns[1].hidden = btns[2].hidden = true;
        }
...
/*      var stat = document.querySelector('.cbi-modal [data-name="_wifistat_modal"] .ifacebadge.large');
    ...
    handleEnabled: function(m, ev, value) {
      var num = ev.substring(5, 6);
 
  //    console.log("handleEnabled: ev: " + ev + " value: " + value + " num: " + num + " was: " + enabledVar[num]); 
      
      if (value != enabledVar[num]) {
        enabledVar[num] = (value == "true");
        
    //    console.log(enabledVar[num] + " " + enabledVarOld[num]);
       
        var disabled = (value == "true") ? "" : "true";
        
//        console.log("Disabled: " + disabled);
   
        var htmode       = document.getElementById("widget.cbid.wireless." + ev + ".htmode");
        var ssid         = document.getElementById("widget.cbid.wireless." + ev + ".ssid"); 
        var encryption   = document.getElementById("widget.cbid.wireless." + ev + ".encryption");  
        var key          = document.getElementById("widget.cbid.wireless." + ev + ".key");  
        var serverIP     = document.getElementById("widget.cbid.wireless." + ev + ".serverIP");  
        var serverPort   = document.getElementById("widget.cbid.wireless." + ev + ".serverPort");  
        var serverSecret = document.getElementById("widget.cbid.wireless." + ev + ".serverSecret");  
      
        htmode.disabled       = disabled;
        ssid.disabled         = disabled;
        encryption.disabled   = disabled;
        key.disabled          = disabled;
        serverIP.disabled     = disabled;
        serverPort.disabled   = disabled;
        serverSecret.disabled = disabled;
      }
    },

        
    handleChannel: function(m, ev, value) {
        //console.log("handleChannel"); 
        var num = ev.substring(5, 6); 
         
        channelVar[num] = value; 
     },
 
    
    handleSSID: function(m, ev, value) {
       //console.log("handleSSID"); 
       var num = ev.substring(5, 6); 
    ...
    handleEncryption: function(m, ev, value) {
      var iface  = "default_" + ev;
      var itemName = iface + 'encry_sel';
      //console.log("handleEncryption, value:" + value);

      var num = ev.substring(5, 6);
      
      encryptVar[num] = value;  
    ...
    handleMode: function(m, ev, value) {
      var num = ev.substring(5, 6);
    ...
    handleKey: function(m, ev, value) {
      ...
      //console.log("handleKey: " + value); 
    },
    
    handleServerIP: function(m, ev, value) {
        var num = ev.substring(5, 6);
        authserverVar[num] = value;
    },
    
    handleServerPort: function(m, ev, value) {
        var num = ev.substring(5, 6);
        authportVar[num] = value;
    },
        
    handleServerSecret: function(m, ev, value) {
        var num = ev.substring(5, 6);
        if (value.length >= 10) {           
          authsecretVar[num] = value;
        }
    },
    ...
    renderSimple: function() {
        ...
        s = m.section(form.TypedSection, 'wifi-device' /*, _('Wireless Overview')*/);        
        s.anonymous = true;        
        s.addremove = false;
        ...
        s.load = function() {
            return network.getWifiDevices().then(L.bind(function(radios) {
              ...
              //console.log("radios: " + this.radios.length + " wifis: " + this.wifis.length);
 
/*              for (var radio = 0; radio < this.radios.length; radio++) {
                 console.log("name: " + this.radios[radio].getName());
              }*/
              
  
              for (var wifi = 0; wifi < this.wifis.length; wifi++) {
                var name      = this.wifis[wifi].getName();
                var frequency = this.wifis[wifi].getFrequency()
                
                // Globals; could be a map.
                
                if (name == "default_radio0") {
                  radioFrequencies[0] = frequency;
                } else {
                  radioFrequencies[1] = frequency; 
                }
                
                 //console.log("name: " + this.wifis[wifi].getName() +  " freq: " +
                  //this.wifis[wifi].getFrequency()); 
              }
        ...
        o = s.option(form.SectionValue, 'values', form.NamedSection, 'values');
        
        
        o.render = L.bind(function(view, section_id) {
          var device = section_id;
          var num = device.substring(5, 6);
          
          //console.log("num: " + num);
          
          var table = E('table', { 'class': 'cbi-section' });
          var band = uci.get("wireless", device, "band");
          var name = (num == 0) ? "WiFi 2.4 GHz Settings" : "WiFi 5 GHz Settings";
         
          var iface  = "default_" + device;
  
          // server (none)      RADIUS authentication server to handle client authentication
          ...
          enabledVar[num] = enabledVarOld[num] = (uci.get("wireless", device, "disabled") == "1") ? false : true;
          bandVar[num]    = bandVarOld[num] = band;
       
          ssidVar[num]    = ssidVarOld[num] = uci.get("wireless", iface,  "ssid");
          channelVar[num] = channelVarOld[num] = uci.get("wireless", device, "channel");
          modeVar[num]    = modeVarOld[num] = uci.get("wireless", device, "htmode");
          
          keyVar[num]     = keyVarOld[num] = uci.get("wireless", iface,  "key");

          var itemName = iface + 'encry_sel';
          var encySelVal = sessionStorage.getItem(itemName);
          //console.log("encySelVal:" + encySelVal);
          var encySelEnterprise = false;          
          if ((encySelVal == null) || (encySelVal == '')) {
            encryptVar[num] = encryptVarOld[num] = uci.get("wireless", iface,  "encryption");
          } else {
            encryptVar[num] = encySelVal;
            sessionStorage.setItem(itemName, "");
          }
          //console.log("encryptVar[num]:" + num + " " + encryptVar[num]);
          if ((encryptVar[num] == "wpa2+ccmp") || (encryptVar[num] == "wpa3") || (encryptVar[num] == "wpa3-mixed")) {
            encySelEnterprise = true;          
          }

          authserverVar[num] = authserverVarOld[num] = uci.get("wireless", iface,  "server");
          authportVar[num]   = authportVarOld[num] = uci.get("wireless", iface,  "port");
          authsecretVar[num] = authsecretVarOld[num] = uci.get("wireless", iface,  "auth_secret");
          
          var cm = new form.Map('wireless', name);
  
          var s = cm.section(form.NamedSection, section_id);   
            
          o = s.option(form.ListValue, 'deviceDisabled', _('Radio Enabled'));
          o.value("true",  "Enabled");
          o.value("false", "Disabled");
          o.default = enabledVar[num] ? "true" : "false";
          o.onchange = L.bind(this.handleEnabled, this);
              
          var label = _('Channel');
          var freq = radioFrequencies[num];
          
          if (freq) label += " (" + freq + " GHz)";

          o = s.option(form.ListValue, 'channel', label);
          o.value("auto", "Auto");
          o.default  = channelVar[num];
          o.readonly = !enabledVar[num];
          o.onchange = L.bind(this.handleChannel, this);
          
          var startChannel, endChannel, incChannel;
          var startFreq, incFreq;
          ...
          if (num == 0) {
            ...
            // TM: Updates to address Jira https://assured-wireless.atlassian.net/browse/AW12FI-140
            // Add Channels 149-161 to 5Ghz Wireless selection
            for (var channel = startChannel; channel <= endChannel; channel += incChannel) {
                o.value(channel,  channel + " (" + startFreq + " Mhz)");
                startFreq += incFreq;
            }
            startChannel = 149;
            endChannel = 161;
          ...
          o = s.option(form.ListValue, 'htmode', _('Mode'));
          o.readonly = !enabledVar[num];
          ...
          o.onchange = L.bind(this.handleSSID, this);
          o.default  = ssidVar[num];
          o.readonly = !enabledVar[num];
          
          o = s.option(form.ListValue, 'encryption', _('Encryption'));
          o.onchange = L.bind(this.handleEncryption, this);
          
          // TM: Updates to address Jira https://assured-wireless.atlassian.net/browse/AW12FI-152
          // Update WiFi Encryption Options
          o.value("psk2",           "WPA2-PSK");
          // There is a Jira entry, https://assured-wireless.atlassian.net/browse/AW12FI-153
          // Add Enterprise options to WiFi Encryption selection
          // which states that these lines should be uncommented in a future release.
          o.value("wpa2+ccmp",       "WPA2-EAP");
          o.value("wpa3",            "WPA3-EAP");
          o.value("wpa3-mixed",      "WPA2-EAP/WPA3-EAP");
          o.value("sae-mixed",       "WPA2-PSK/WPA3-SAE");
          o.value("sae",             "WPA3-SAE");
          o.value("none",            "Disabled");
          o.default = encryptVar[num];
          
          o.readonly = !enabledVar[num];
          
          if (encySelEnterprise == false) {
            o = s.option(form.Value, 'key', _('Key'));
            o.onchange = L.bind(this.handleKey, this);
            o.default  = keyVar[num];
            o.readonly = !enabledVar[num];
            o.rmempty = true;
            o.password = true;
          ...
          // Enterprise Encryption modes are wpa2, wpa3 and wpa3-mixed
          if (encySelEnterprise == true) {
            o = s.option(form.Value, 'serverIP', _('RADIUS Server IP'));
            o.onchange = L.bind(this.handleServerIP, this);
            o.default  = authserverVar[num];
            o.readonly = !enabledVar[num];
            
            o = s.option(form.Value, 'serverPort', _('RADIUS Server Port'));
            o.onchange = L.bind(this.handleServerPort, this);
            o.default  = authportVar[num];
            o.readonly = !enabledVar[num];
                        
            o = s.option(form.Value, 'serverSecret', _('RADIUS secret'));
            o.onchange = L.bind(this.handleServerSecret, this);
            o.default  = authsecretVar[num];
            o.readonly = !enabledVar[num];
            o.rmempty = true;
            o.password = true;
          ...
          cm.render().then(function(node) {
            table.appendChild(E('tr', { 'class': 'tr' }, [
                E('td', { 'class': 'td left', 'width': '60%' }, [ node ])
            ]));
          }); 
            
          cm.save = L.bind(function() {
           
            for (var num = 0; num < 2; num++) {

              var radio  = "radio" + num;
              var device = "default_" + radio;
  
              
              if (enabledVar[num] != enabledVarOld[num]) {
                uci.set('wireless', radio, 'disabled', enabledVar[num] ? "" : "1"); 
                enabledVarOld[num] = enabledVar[num]
              }
              
              if (channelVar[num] != channelVarOld[num]) {
                  uci.set('wireless', radio, 'channel', channelVar[num]); 
                  channelVarOld[num] = channelVar[num]
              }
                
              if (ssidVar[num] != ssidVarOld[num]) {
                uci.set('wireless', device, 'ssid', ssidVar[num]); 
                ssidVarOld[num] = ssidVar[num]
              }
              
              if (encryptVar[num] != encryptVarOld[num]) {
                uci.set('wireless', device, 'encryption', encryptVar[num]); 
                encryptVarOld[num] = encryptVar[num]
              }

              uci.remove('awc', num, 'encryption_sel'); 
              
              if (keyVar[num] != keyVarOld[num]) {
                uci.set('wireless', device, 'key', keyVar[num]); 
                keyVarOld[num] = keyVar[num]
              }
              
              if (modeVar[num] != modeVarOld[num]) {
                uci.set('wireless', radio, 'htmode', modeVar[num]); 
                modeVarOld[num] = modeVar[num]
              }

              if (authserverVar[num] != authserverVarOld[num]) {
                uci.set('wireless', device, 'server', authserverVar[num]);
                authserverVarOld[num] = authserverVar[num]
              }

              if (authportVar[num] != authportVarOld[num]) {
                uci.set('wireless', device, 'port', authportVar[num]);
                authportVarOld[num] = authportVar[num]
              }

              if (authsecretVar[num] != authsecretVarOld[num]) {
                uci.set('wireless', device, 'auth_secret', authsecretVar[num]);
                authsecretVarOld[num] = authsecretVar[num]
              }
...
/*      s.renderRowActions = function(section_id) {
        ...
        s.addModalOptionsDummy = function(s) {
...
/*          if (inst.getWifiNetworks)
      ...
      //      var btns = this.section.querySelectorAll('.cbi-section-actions button');
        //    console.log("buttons: " + section_id);
        ...
        return m.render().then(L.bind(function(m, nodes) {
          ...
          var btns = nodes.querySelectorAll('.cbi-section-actions button');
          
           console.log("buttons: " + btns.length);
          
//           nodes.editable = false;
           
        /*   for (var button = 0; button < btns.length; button++) {
             btns[button].disabled = true;
             btns[button].visible = true;
             //btns[button] = null;
           }*/
