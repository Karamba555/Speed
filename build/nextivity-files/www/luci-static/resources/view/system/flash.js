...
function handleShutdown(m, ev, value) {
...
return view.extend({
	load: function() {
		var tasks = [
			...
			L.resolveDefault(L.uci.load('awc')),
            L.resolveDefault(L.uci.load('network')),      
			L.resolveDefault(fs.read('/tmp/nextivity/software_updates'), {})
		];
    ...
    getButtonTitle: function(value) {
	  if (value == "poe") 
		  return "Flash Power over Ethernet MCU";
	  else if (value == "vehicle")
		  return "Flash Vehicle MCU";
	  
	  return "Flash Custom MCU...";
	},
	
	handleMCU: function(m, ev, value) {
//		console.log("handleMCU: " + value);

		var buttonElement = document.getElementById("cbi-json-actions-mcu"); 
		if (buttonElement != undefined) {
		   // Yes, yes. 
			
		   var oldText = this.getButtonTitle(flashMCUValue);
...
//		   console.log("old: " + oldText + " new: " + newText);
           ...
           buttonElement.onclick = L.bind(this.handleMCUFlash, this);
		} 
		
		flashMCUValue = value;
		
        this.setShutdownEnabled(flashMCUValue == "vehicle");
        
		//console.log("HTML: " + HTML.innerHTML);		
	},

	spinnerMCU: function(param) {

        if (!confirm(_('Flashing the MCU takes about 4 minutes.\nThe MegaFi will reboot after.')))
          return;
		...
		// Flashing takes about 3 1/2 seconds
		var timeout = (4 * 60);
		...
		try { 
			...
			fs.exec('/sbin/mcu-flash.sh', [ '-d' , param ]).catch(function(error) {
	...
	handleOK: function(ev) {
		//console.log("hideModal");
		ui.hideModal();
	},
	
	handleMCUFlash: function(ev) {

	//	console.log("handleMCUflash");
		var currValue = L.uci.get('awc', 'mcu', 'mcu_application');
		
		if ((currValue != "custom") &&
			(flashMCUValue == currValue)) {
			
			var body = [];
		    var force = E('input', { type: 'checkbox' });
			
			body.push(E('p', { 'class': 'alert-message',
				               'click': ui.createHandlerFn(this, 'handleOK', force)
						}, 
						[
						_('MCU firmware is already set to ' + 
                          (currValue == 'poe' ? "Power over Ethernet" : "Vehicle") 
                        )
						]));
			
			ui.showModal(_('Flash MCU'), body);
		...
		if (flashMCUValue == "custom") {
			return ui.uploadFile('/tmp/mcu.bin', ev.target.firstChild)
				.then(L.bind(function(btn, reply) {
					this.spinnerMCU('/tmp/mcu.bin');
					//console.log("handleMCUFlash: " + L.env.rpctimeout);
					//return fs.exec('/sbin/mcu-flash.sh', [ '/tmp/mcu.bin' ])
				}, this, ev.target));
		} else {
			//console.log("handleMCUFlash: " + L.env.rpctimeout);
			//return fs.exec('/sbin/mcu-flash.sh', [ flashMCUValue ])	
			this.spinnerMCU(flashMCUValue);
		}
	},
    
    setShutdownEnabled: function(enabled) {
       var dropdown = document.getElementById("shutdown-values");
...
*/
	...
	render: function(rpc_replies) {
		var has_sysupgrade = (rpc_replies[0].type == 'file'),
			...
			firmware_updates = rpc_replies[6],
...
//		var firmware_updates
		
//		if (rpc_replies[6] !== false) 
//			firmware_updates = rpc_replies[6];
...
/*
		...
		o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('Available Firmware'),
				_('Firmware available in the Cloud'));
		ss = o.subsection;
		
//		console.log("Updates: " + firmware_updates);
		
		var none = true;
		
		try {
		  var updates = JSON.parse(firmware_updates);
		
  		  if (updates["ei.action"] !== false) {
		    var update = updates["ei.action"]["software.update"];
			
		    if (update !== false) {
			  var paths = update.split('/');
			 
			  if (paths[2] !== false) {
				var version = paths[2];
			  
//  		    console.log("update: " + update);
				o = ss.option(form.Button, 'softwareupdate');
			    o.inputstyle = 'action important';
			    o.inputtitle = _('Version ' + version);
			    o.onclick = L.bind(this.handleVersionUpdate, this);
	    ...
	    if (none) {
				o = ss.option(form.Button, 'softwareupdate');
			    o.inputstyle = 'action important';
			    o.inputtitle = _('None Available');
				o.disabled = true;
		}
		
		var skin = L.uci.get('awc', 'main', 'skin');
        
        if (skin != "airgain" && "MegaFi" == "EI") {
  		  o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('Flash new MCU firmware'),
				_('Upload Custom MCU firmware.'));

          ss = o.subsection;

		  flashMCUValue = L.uci.get('awc', 'mcu', 'mcu_application');
		
//		console.log("MCU VAL: " + flashMCUValue);
		
		  o = ss.option(form.ListValue, 'mcu_firmware', _('MCU Firmware'));
		  o.value("vehicle",  "Vehicle");
          o.value("poe",      "Power over Ethernet");
		  o.value("custom",   "Custom");
		  o.default = flashMCUValue;
		  o.onchange = L.bind(this.handleMCU, this);

          var shutdown_val = L.uci.get('awc', 'mcu', 'shutdown_period');

          o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions');
          
          o.render = L.bind(function(view, section_id) {
          //  console.log("Render shutdown");  
            ...
            o = s.option(form.ListValue, 'shutdown', _('Vehicle Shutdown Period'));
            
		    o.value("12", "12 Seconds");
            o.value("900", "15 Minutes");
		    o.value("3600", "1 Hour");
		    o.default  = shutdown_val;
		    //o.onchange = L.bind(this.handleShutdown, this);
            o.onchange = handleShutdown;
      
            var style = 'margin-right:2em;margin-left:2em';
            
            if (flashMCUValue != "vehicle") style += ";display:none";
          ...
          o = ss.option(form.Button, 'mcu', _('Image'));
          o.inputstyle = 'action important';
          o.inputtitle = this.getButtonTitle(flashMCUValue);
			//_('Flash MCU binary...');
          o.onclick = L.bind(this.handleMCUFlash, this);
        }
